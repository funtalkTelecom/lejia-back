...
$(document).on("focus","#batchAgent",function(){
    autocomplete(this)
});
...
$(document).on("click","#batch-add .modal-footer .btn-success",function() {
    if ( !$("#batch-add input[type=file]").val()){
        alert("请选择文件");
        return
    }
    if ( !$("#batch-add input[ name=agentId]").val()||!$("#batchAgent").val()){
        alert("请选择代理商");
        return
    }
    var options = {
        type : "post",
        url: "numprice/batch-add-numprice",
        success : function(data) {
            dataList.load();
            $('#batch-add').modal('hide');
            alert("数据已提交，详见稍后下载的excel表格");
            if (isIE()){
                window.open("get-file/batch_add_numprice/"+data.data);
            }else {
                window.location.href="get-file/batch_add_numprice/"+data.data;
            }
        }
    };
    // 将options传给ajaxForm
