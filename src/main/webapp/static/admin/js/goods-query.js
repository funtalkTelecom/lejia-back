...
$(function() {
                ...
                "cm" : [{
					...
					"renderer":function(v,record){
                        ...
                        $operate.find(".update").click(function () {
                            $("#goodsInfo .modal-title").html("详情");
                            $("#checkTrue").hide();;
                            $("#checkFalse").hide();
                            $("#skuCheckResult").hide();
                            $("#saleNum").attr('readonly', 'readonly');
                            isDelAllSku = false;
                            //获取仓库商品类型下拉框
                            $.ajax({
                                type: "POST",
                                ...
                                success: function(data){
                                    if(data.code=="200"){
                                        ...
                                        if(IsShowProduct==1){
                                            titleStrObj.skuRepoGoods.isShow=false;
                                        }
                        ...
                        $operate.find(".check").click(function () {
                            ...
                            $.post("goods/goods-info", {gId: v}, function (data) {
                                ...
                                $('#goodsInfo').modal('show');
                                $("#skuCheckResult").show();
                                $.post("sku/sku-list-num", {gId: v}, function (data) {
                                    var res = data.data;
                                    var htm=""
                                    for(var i=0;i<res.length;i++){
                                        htm +='<tr align="center" height="27">'+
                                        '<td nowrap=""><strong>'+res[i].yys+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].jb+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].resource+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].sku_tob_price+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].yj+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].ej+'</strong></td>'+
                                        '<td nowrap=""><strong>'+res[i].sj+'</strong></td>';
                                    }
                                    $("#skuNumListTbale").html(htm);
                                }, "json");
                            }, "json");
                        });
    ...
    $(document).on("click","#checkTrue",function() {
        $.post("goods/goods-check-true", {gId: $("#gId").val()}, function (data) {
            if(data.code==888){
            }else{
                dataList.load();
                $('#goodsInfo').modal('hide');
    ...
    $(document).on("click","#checkFalse",function() {
        if (confirm("审核不通过系统会自动下架该商品，请确定")) {
            $.post("goods/goods-unsale", {gId: $("#gId").val()}, function (data) {
                dataList.reload();
                alert(data.data);
                $('#goodsInfo').modal('hide');
            }, "json");
    ...
    function createTile(){
        if(IsShowProduct==1){
            titleStrObj.skuRepoGoods.isShow=false;
            // titleStrObj.skuRepoGoods.type=-1;
        }
        $("#gProperty").find("div.row").each(function(i, e){
    ...
    $("button[data-target=#goodsInfo]").bind("click", function () {
        var now1 = new Date();
        var day2 = new Date();
        day2.setDate(day2.getDate() + 365);
        var s2 = day2.format("yyyy-MM-dd hh:mm:ss");
        $('#gStartTimePicker').val((new Date()).format("yyyy-MM-dd hh:mm:ss"));
        $('#gEndTimePicker').val(s2);
        gIsAucOnClick();
        //重置类别
        gType2.change();
        gType1.change();
    ...
    $("#opengoodsInfo").click(function(){
        ...
        if(gid==""){
            $("#btn").show();
            $("#goodsInfo .modal-title").html("添加商品");
            $("#checkTrue").hide();;
            $("#checkFalse").hide();
            $("#saleNum").removeAttr('readonly');
        }
    });

    $("#automaticShelves").click(function(){
        if (confirm("确定要自动上架吗")) {
            $.post("goods/goods-automatic-shelves", function (data) {
                alert(data.data);
                dataList.reload();
            }, "json");
        }
...
var titleStrObj = {
    ...
    "skuGoodsType":{
        ...
        "type":'<select onchange="skuIsNumChange(this)" tag="sku_skuindex" name="skukey" selectValue="skuvalue"><option value="-1">请选择...</option><option value="1">白卡</option><option value="4">超靓</option></select>',
        "titleClass":""
    },
    "skuRepoGoodsName":{
        "isShow":false,
    ...
    "skuRepoGoods":{
        "isShow":true,
        "title":"关联仓库商品",
        "type":'<select class="chosen-select" tag="sku_skuindex" name="skukey" selectValue="skuvalue"><option value="-1">请选择...</option><option value="1">白卡</option><option value="2">成卡</option><option value="3">普卡</option></select>',
        "titleClass":""
    },
    "skustatusText":{
        "isShow":true,
