...
$(function() {
	...
	dataList = new $.DSTable({
		...
		"cm" : [{
					...
					"renderer":function(v,record){
						...
						if(p_addNum) {
							node.push('<a class="btn btn-success btn-xs addNum" href="javascript:void(0);">设置号码</a>')
                        }
                        ...
                        $operate.find(".addNum").click(function () {
                            $("#mid").val(v);
                            $.post("meal/meal-list-num", {mid: v}, function (data) {
                                if (data.code == "200") {
                                    var _data = data.data;
                                    $("#saleNums").val(_data);
                                }
    ...
    $(document).on("click","#saleNumInfo .modal-footer .btn-success",function() {
        var saleNums = $("#saleNums").val().replace(/\s+/g,"");
        if(saleNums==""){
            alert("请填写号码");
            return false;
        }
        $.post("meal/meal-add-num",$("#saleNumInfo form").serialize(),function(data){
            if (data.code == "200") {
                dataList.reload();
